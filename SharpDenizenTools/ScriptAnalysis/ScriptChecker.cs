...
namespace SharpDenizenTools.ScriptAnalysis
{
    ...
    public class ScriptChecker
    {
        ...
        public static readonly Dictionary<string, KnownScriptType> KnownScriptTypes = new()
        {
            ...
            { "dialog", new KnownScriptType() { RequiredKeys = ["dialog_type"], ValueKeys = ["dialog_type", "title", "body", "closeable"], ListKeys = ["inputs.*", "items.*"], ScriptKeys = ["button.*", "yes_button.*", "no_button.*", "buttons.*"], LikelyBadKeys = ["script", "actions", "steps", "events"], Strict = false, CanHaveRandomScripts = false } }
        };
